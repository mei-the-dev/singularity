...
export default function IssueCard({ issue, onOpen }: { issue: Issue; onOpen?: (id: string) => void }) {
  ...
  return (
    <motion.div
      ...
      className="glass-panel p-4 rounded-lg cursor-grab active:cursor-grabbing focus:outline-none focus-visible:ring-4 focus-visible:ring-cyan-400/60 transition-shadow duration-180 ease-in-out hover:shadow-2xl"
      onDoubleClick={() => onOpen && onOpen(issue.id)}
      style={{ touchAction: 'none', color: 'var(--gold)', background: 'linear-gradient(180deg, rgba(20,20,20,0.8), rgba(8,8,10,0.7))', filter: 'drop-shadow(0 8px 24px rgba(212,175,55,0.05))' }}
      role="button"
      tabIndex={0}
