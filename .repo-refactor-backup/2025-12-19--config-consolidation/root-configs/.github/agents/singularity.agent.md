...
- **start_service** — Launch background processes (UI/Server). Auto-kills zombie ports.
  ...
  - **Note:** `start_service` runs commands from the repository root (`repoRoot: singularity`) by default; use `MCP_REPO_OVERRIDE` if you need a different root.
