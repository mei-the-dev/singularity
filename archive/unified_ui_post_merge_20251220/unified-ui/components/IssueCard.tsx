...
export interface IssueCardProps { issue: Issue; onClick?: () => void }
...
export const IssueCard: React.FC<IssueCardProps> = ({ issue, onClick }) => {
  ...
  const getTypeIcon = (type: Issue['type']) => {
    switch (type) {
      case 'bug': return <GitCommit className="w-3 h-3" />;
      case 'feature': return <GitPullRequest className="w-3 h-3" />;
      case 'enhancement': return <GitCommit className="w-3 h-3" />;
